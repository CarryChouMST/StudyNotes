...
- 叉乘的行列式
  ...
  >$\left[\begin{matrix} i&&j&&k \\ u1&&u2&&u3 \\ v1&&v2&&v3  \end{matrix}\right]$ 
  >
  >为什么是这个？
...
- ==左二分查找==指定值在有序容器中的下标。==除开真的小于外，即使等于也是右边动，直到两个指针汇合。==

- ==注意二分查找的有效取值范围为0~size(), 有可能超过容器最大Index【当查找值为最大值时】，因此拿到的Idx不能直接从容器中取值看是否重复！==

  ~~~cpp
  
  template <class _T, class _Compare>
  Guint64 lower_bound(Utility_In AnGeoVector<_T> &A_Vec, Guint64 start, Guint64 end, Utility_In _T &val, _Compare __comp)
  {
      Guint64 __len = end - start;
      while (__len > 0)
      {
          Guint64 __half = __len >> 1;
          const auto &__middle = A_Vec[start + __half];
          if (__comp(__middle, val))
          {
              start = start + __half + 1;
              __len = __len - __half - 1;
          }
          else  // 除开真的小于外，即使是等于也是右边动，最后返回左边。
              __len = __half;
      }
      return start;
  }
...
- C++在容器中，删除多个指定元素，可以在O(n)时间内完成，具体实现为：转移元素将待删除元素全部放在末尾，其他元素依序向前，最后一起将末尾元素删除。

  ~~~cpp
  void remove(value_param_type val)
      {
          _value_type *pFirst = GetFirst();
          _value_type *const pLast = GetLast();
          _value_type *pWhere = pFirst;
          _value_type *pNext = pWhere;
          while (pNext < pLast)
          {
              if (*pNext != val)
              {
          ...
          while (pNext > pWhere)
              (--pNext)->~_value_type();
          if (small_size_enabled)
              ReduceSmallSize(pWhere, pWhere - pFirst);
          else
              m_pLast = pWhere;
      }
  ~~~
...
- 无向图：并查集、DFS；有向图：拓扑排序、DFS

- ==无向图在找环问题上，还不能用有向图存储，无向图中两节点的回环需要两条无向边，而有向图中只需要两条有向边，对应只需要一条无向边==。

- 有向图的强连通分量，实际上是一个极大环，里面可能存在子环。

- 环和连通的区别就在于使用的边的数量。单向连通一定弱连通，弱连通不一定成环（对无向图，一条边可以走两次），强连通成环（对有向图，一条边只能走一次）

- ==取所有环主要是如何在比较少的遍历中构建多个可能有重合节点的环，tarjan算法会找一个强连通分量，逐步断去每一条边，然后尝试从边的终尝试走向起点。==|==进一步的，在找到强连通分量后，可以通过dp记录多源点对路径表，最后对一个多源点对的两个方向，求笛卡尔积就行。==

- tarjan(塔扬算法)和 `Kosaraju` 算法

- tarjan算法能在扫一次就把所有强连通分量构建的前提是，认为强连通分量在搜索树中一定属于某个根节点下的子树，这样在回溯时，一个强连通分量被认为一定属于一个不重叠的连续区域，因此可以用栈直接模拟。**回溯时，如果在环内，且不为源点，会暂停弹出；而如果不在环内，自己为自己的源点，会立即弹出，本质上就是逐子树处理。Tarjan 算法可以处理重边和自旋。多重边和自旋不会影响算法的正常运行和结果。算法的设计和逻辑是基于节点之间的连通性，而不是边的数量。**

- 【tarjan算法】dfn是访问次序，low是能连通的最小次序点（在树中更接近根节点），如果dfn==low了，表示分支中该节点不可能反向访问了（访问到的low就是他自己），此时将未回收栈上的所有对象一起构成极大连通分量（还在栈上的对象都是会反向访问的）。 **一般实现时使用染色法，完全未访问用红色，在路径中，但是子树递归还未结束用黄色（防止子节点又访问过来了，告诉他这是根节点），子树递归已经结束，出栈的用蓝色。**

- 【tarjan算法求割点】在求SCC时，因为是有向图，所以没有往父节点访问的问题。在求无向图的割点时，low数组的下标表示顶点的编号，数组中的值表示DFS中该顶点**不通过**父顶点能访问到的祖先顶点中最小的顺序值（或者说时间戳），==这里要求不通过父定点访问的==。==沿树边(DFS下去的节点)和回边（已经访问的，但不是父节点的节点）到达的时间戳==

- 

- tarjan算法，可以用于寻找强连通分量，即图中的极大环，但并不能替代找环，因此极大环中可能存在嵌套环的情况。【**注意识别前向边、分叉边。对于在栈内，或者标黄的节点，属于前向边，要取low的最小值，而不能取DFN，不然可能导致栈无法退出。因为你直接取DFN了都没有比较最小值，在成环情况下，有可能自己的LOW[u]比找到的v要小，因为你的v可能是通过u的其他边间接找到。**】
  ...
  tarjan(u){
    ...
    for each (u, v) in E                       // (3) 枚举每一条边
      if (v is not visted)                     // (4) 如果节点v未被访问过
        ...
        LOW[u] = min(LOW[u], LOW[v])           // (8) ==注意这里一定要取两个low的最小值==
  ...
  bool GraphList::FindSCC(int index, int *dfn, int *low, int &tStamp, stack<int> &aStack){
  	...
  	for(Edge e = FirstEdge(graList[index].vertexId); e.to >= 0; e = NextEdge(e)) {
  		...
  		if(mark[j] == UNVISITED) { //UNVISITED没有访问过
  			...
  			// low[index] = dfn[j];
              low[index] = std::min(low[index], low[j]);  // 这里要取两个的较小值，因为是前向比边
  		}
  	}
  	//回溯
...
- 【tarjan找割边】tarjan的核心是dfs和回溯中的dfn和low值的选取，一个是遍历次序，一个访问到的最接近根节点的点。割边和割点的核心在于，对应的边的出去的节点的low，不能大于该边进来节点的dfn，==但注意不能是重边==。

- 有向图找环，可以使用tarjan算法找到强连通分量后，dfs找环；无向图找环，则可以使用并查集找到连通分量后，最每个边数>=点数的连通分量使用dfs找环。注意在使用dfs找环时，可以断掉一条边，然后尝试从终点找到起点，但是要注意中间不要经过环。这样就变为了多源点对之间的所有路径，这个问题是指数级的，因为解是指数级的, 所以算法最低也是指数级的，V*m^n, 其中V是节点数，m是点的度数，n是最小生成树的深度。
...
- Floyd算法
  ...
  - 弗洛伊德属于动态规划的算法剖析【**动态规划刷表+滚动数组优化**】：
    ...
    - 状态转移：d\[k+1\]\[i\]\[j\]中有可能有两种情况：最短路经过k+1/不经过k+1，则有$d[k+1][i][j] = min(d[k][i][j], d[k][i][k+1] + d[k][k+1][j]) (k,i,j \in [1, n])$
    - 滚动数组优化，复用$d[i][j]$，就变成了三层循环+松弛操作。
    
  - 如果要找floyd具体的最短路路径，可以存一个dis矩阵和一个path矩阵，dis矩阵存最终结果，用滚动数组优化。最外层遍历中间点，实际上就是对每一个点对，激活该中间点的入度边和出度边。

  - ==注意floyd的path矩阵实际上是记录从i到j路径的倒数第二个节点k, 一般称为pre矩阵，也就是i->k->j, 然后真正取路径时需要做树的路径拼接。==

  - 从另一个角度上说，如果需要拿所有的路径，pre矩阵上的值应该是所有可能的pre数组【==注意剪枝，路径中不要有环==】，这样就层次化的描述了一棵树，做dfs即可搜索出所有的路径。

  - 注意，基于floyd拿出所有的路径复杂度为指数级，因为其解是指数级，因此无论怎么复用结果，算法一定是指数级的，一般可以认为是$m^n$，其中m是点的度，n为最小生成树的深度。
...
- **注意拓扑排序在构建入度map时要考虑有没有重边，如果有重边可能，在插入出边时，要根据insert后的.second参数为true/false决定是否增加自己的入度。**
  
- **时间复杂度**：O(n+m) 因每个结点只需加入队列一次，而每个结点加入队列前需要进行入度数量次的操作，因此时间复杂度为
  O(n+m)。【实际上就是初始化入队后，每个节点都要扫一遍自己的入度，==对于测是否有环的情况，可以加一个计数器，队列出队一次计数，出循环后计数不等于点数则成环==】
...
- 把一个图的顶点划分为两个不相交集U和V，使得每一条边都分别连接U、V中的顶点。如果存在这样的划分，则此图为一个二分图，一个等价定义是**不含有「含奇数条边的环」的图**。（实际上就是把所有的节点抑制某些边后层次化，基于森林层次奇偶性分组，组内不能有内连边。|==原来我的这种想法有点像图搜搜染色==。[示例图](.算法相关/image-20241212134921124.png)

- 具体来说，判断二分图有三种方法：深搜，广搜和并查集。

  - 其中图搜索就是选定一个起点，按连接边进行染色（注意去重剪枝），节点只能染成红色/蓝色，且相邻节点颜色不同。如果能顺利将所有节点染色完毕，则该图为二分图，反之则不是。

  - 【==250508|二分图上每个顶点的**所有邻接点都应该属于同一集合**，且不与顶点处于同一集合。==】并查集主要是将当前顶点的所有邻接点合并，合并前先判断邻接点与当前顶点是否已经在一个集合中，如果已经在了就直接失败。【==或者是按照邻接点合并的逻辑（注意不是原始的关联边两端，而是把邻接点合并，枢纽点不合并）， 所有顶点最终可以被分为>=2个集合==】。
    ...
    bool isBipartite(vector<vector<int>>& graph) {
        ...
        for(int v = 0; v < n; ++v) {
            for(int neighbor = 0; neighbor < graph[v].size(); ++neighbor) {
                if(isConnection(v, graph[v][neighbor])) {  // 校验枢纽点
                    return false;
                }
                merge(graph[v][0], graph[v][neighbor]);  // 合并邻接点
            }
        }
        return true;
    }
...
- 【增广路算法、匈牙利算法、Kuhn-Munkres算法】基于DFS找增广路，将二分图的边定向，变为基于dfs找有向图的一条指定规则的路径。时间复杂度O(VE)，遍历所有点，每个点遍历所有边找增广路。如果找到增广路，则将增广路上的所有边取反（非匹配变匹配、匹配变非匹配），**实现上可以通过递归+回溯实现。另外由于二分图是一种结构化的组织形式，在mch数组中可以只存右部图的节点、而在使用邻接表记录边时，只基于左部图记录。**|==枚举所有未匹配点，寻找增广路，事实上点只用访问一次，因此一个未匹配点在寻找增广路后，会变为匹配点（匹配点不会改变或减少，只会增加）。==
...
- AABB动态分裂二叉树
  ...
  以每一个节点空间内节点数目为阈值，节点数目超过阈值则采用最长轴方法确定分离轴，使构建的AABB大致平衡，后续可以采用并行碰撞。

- 分离轴碰撞：如果要看两个多边形是否碰撞，多边形维度较少，但维度之间长度差异大（比如矩形道路），可以考虑使用轴分离算法，投影压缩长维度。

## 贝塞尔曲线

- 贝塞尔曲线完全由控制点决定曲线形状，实际上是递归插值的结果。对于两个点的一阶贝塞尔 ，那就是不同权重下的加权平均；对于二阶，取两条线的加权平均点，然后对这两个点再次进行加权平均。

- 贝塞尔曲线的系数是二项分布的系数，可以理解为在n-1次概率为p的独立二项分布事件下，可能有n种分布，对应于n个控制点坐标，在该概率事件下n-1次的事件期望为参数为p下的插值点。
...
- 如果我们定义，经过某个点，存在一条直线，将平面分为A，B，且多边形完全属于B或属于A的点，称为凸点。==基于凸点使用旋转叉积，可以找到邻接的另一个凸点。==

- 要寻找时针方向（顺/逆）的最值点，正常要有一个基准轴，然后其他点与参考点的连线与该基准轴算角度，得到时针旋转值后排序。而正常使用atan2(y, x)为以x轴为基准轴逆时针的旋转值（-pi, pi）。==凸包这里做了一个优化，对于一个凸点，使用旋转叉积可以得到以凸点参考轴的垂线为基准轴的时针旋转最值。==

  ![image-20250507194722466](.%E7%AE%97%E6%B3%95%E7%9B%B8%E5%85%B3/image-20250507194722466.png)
  ...
  public:
      ...
      vector<vector<int>> outerTrees(vector<vector<int>>& trees) {
          ...
          do
          {
              ...
              for (int r = 0; r < n; ++r)
              {  // 这里找的初始值是最左边的点，因此要找一个点让其他店都在向量右边有唯一解，不需要去重
                  // 就好像最左边有一个墙壁在这里，以当前点为原型进行逆时针甩动，甩动的极值就是所有点都在左边的点。
                  // 注意，以一个凸点为起点，下面这种旋转叉积就可以找到下一个极值凸点
