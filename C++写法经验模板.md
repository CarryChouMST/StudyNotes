...
- 【enum class】强类型枚举与整形加减

  ~~~cpp
  // enum class 枚举与整型数进行加减运算，你可以通过重载加法和减法运算符来实现这个行为。因为 enum class 是一个强类型的枚举，所以不能直接进行加减运算，需要定义全局的运算符重载函数。
  
  // 重载 + 运算符
  A operator+(A e, int value) {
      return static_cast<A>(static_cast<int>(e) + value);
  }
  ...
  // 重载 + 运算符用于与整型相加
  A operator+(int value, A e) {
      return static_cast<A>(value + static_cast<int>(e));
  }
...
- 【递归剪枝】dfs剪枝一般是，外部校验重复，不重复则进入内部插入，退出内部则取出。

- 【遍历删除】遍历删除的一般模式是，用一个新容器存储不删除的对象，然后swap

  ~~~cpp
      AnGeoVector<HDRoadShadowAreaBasePtr> vpAll;
      for (Gint32 i = 0; i < m_AllShadowClosedAreaArr.size(); ++i)
      {
          if (m_AllShadowClosedAreaArr[i]->IsFlagDelete())
              continue;
          vpAll.push_back(m_AllShadowClosedAreaArr[i]);
      }
      m_AllShadowClosedAreaArr.swap(vpAll);
  ~~~
...
- 【模板函数】有部分限制的模板函数
  ...
  ~~~cpp
  	...
  	void ReverseSelf(Utility_In T &A_Line) { ReverseSelf((Gint32)A_Line.size()); }
