...
- C++中双冒号的用法：
  ...
  - 在函数/命名空间外裸加一个::表示从外向内找，从全局往里面找；而如果不加，默认行为为从内向外找。
...
- C++11变长参数模板
  ...
  **变长模板参数还可以用来做一些底层数据流转，可以随意调试修改参数，不一定非得参与什么递归的参数用途：**

  ~~~cpp
  // 在这里可以随时基于调试需要修改调试图元的构造方法，传参构造即可。
  template<typename... Args>
  GLDRetrieveLine* GLDCoupleRoadManager::AddRetrieveEntity(Args&&...args)
  {
  #if ROAD_PLATFORM_WINDOWS
      auto pEntity = new GLDRetrieveLine(std::forward<Args>(args)...);
      m_oVirCombineRetrieve.Add(pEntity);
      return pEntity;
  #else 
      return nullptr;
  #endif
...
- 【C++三角函数】
     ...
     - C++中的math库中的atan2(a,b)是带四象限的反正切，它的取值不仅取决于正切值a/b，还取决于点 (b, a) 落入哪个象限：【==|为什么是(b, a)，是因为反正切是y/x，因此第一个是y，第二个是x==】
  ...
  - 而 atan(a/b) 仅仅根据正切值为a/b求出对应的角度 （可以看作仅仅是2象限反正切）：
     ...
     - C++中的cmath库中提供了三角函数用法，sin、cos、tan、asin、acos、atan等三角函数的输入是弧度，而不是角度。
...
- 【匿名结构体、提升】匿名结构体struct，好处是可以逻辑上分组数据而不增加数据的访问层次，但是由于匿名结构体没有名字，编译器无法单独识别它，无法定义构造函数或赋值运算符。
...
- 【数组与指针】C++中数组名只是可以像指针那样使用，但是编译期内部认识他是一个数组，**数组变量本身保存了数组的大小（即数组元素个数），并且编译器知道这一点。**另外，对于指针，编译器在初始化指针变量时，也在指针变量地址之前分配了一定的前缀，标识了其指针类型。

- 【枚举】C++中enum枚举如果没有指定的话，默认从0开始。

- 在 C++ 的标准库中，对于 `std::map` 的 `rbegin()` 函数，它返回一个反向迭代器，指向 `map` 的最后一个元素的下一个位置，而 `rend()` 指向 `map` 的第一个元素之前的位置。

    因此，`rbegin()` 实际上是从 `end() - 1` 开始的，而不是从 `end()` 开始。

- `#pragma optimize` 是一个编译指示（preprocessor directive），用于控制编译器的优化行为。在你提到的 `#pragma optimize("", off)` 中，它的主要作用是禁用当前源文件或代码块的代码优化。它的参数 `""` 用于表示保持优化等级不变，`off` 表示关闭优化。

- **`swap`:** 通过交换来清空，优雅且能确保内存释放。 std::vector<int>().swap(vec),`std::swap` 函数直接交换两个 `std::vector` 的内部数据结构（指针等），而无需将元素逐个复制，因此是一种非常高效的方法。

- ==C++在类内声明一个静态数据成员时，不能在类内部为其直接提供初始值设定项。静态数据成员的初始化必须在类外进行。这个限制的原因在于静态数据成员属于类本身而非某个具体对象。==C++中静态变量在头文件中声明，在cpp文件中定义的用法：
...
- std::vector的capacity和size概念不一样，一个是容量，一个是尺寸。而vector的数量初始化，实际上是增加了size，即会增加默认元素，不只是capacity。==一般要预先分配一定的数量，便于后面下标赋值，用resize而不是reserve!==

- 指针成员都可以用前置声明，不需要真的引入头文件。

- 假如你的 `A` 类有一个受保护的成员 `b`，那么在类 `A::accessProtected` 内部，你可以基于对应对象和指针访问同一类中其他实例的受保护的成员。**可否访问首先看的是当前上下文环境、第二个是对象的权限。如果在A的上下文环境中，A类对象可以访问A类的任意成员；另外还可以访问对象在外部所能访问到的内容。因此如果是A派生B，在B的上下文环境中，A的对象就不能不访问A类的保护成员了。因为上下文访问权限也只能赋予同类的任意成员。**

- 遍历删除元素，可以倒序遍历删除，这样能保持顺序的稳定，也保证了迭代器的稳定，但是性能比较弱，因为删除一个元素是On的，一般把新的元素挑出来放进新的容器里。

- 联合体的内存是共享的，因此对于非平凡类型的构造函数，构造时不指定成员，编译器不知道是哪个成员要分配这些内存。需要使用placenew。

    ==注意，联合体如果其tag字段由外部struct控制，则外部struct析构时，需要根据该字段帮助联合体释放其潜在持有的资源。==
    
    ~~~cpp
    new(&vec) ... 表示：在联合体的 vec 成员的内存位置上，调用 std::vector<int> 的构造函数。
        union Route {
        enum class Type { Vec, Ptr };
    
        // 默认构造函数：初始化 vec 成员
        Route() : type(Type::Vec), vec() {
            new(&vec) std::vector<int>();  // 显式调用构造函数
...
- std的智能指针如果放在头文件，则要求头文件一定要包含指针定义，不然会产生未定义行为；如果对智能指针做“=”初始化，此时在初始化时会调用移动操作，即一定会有析构，此时头文件没有包含就会有潜在危险。==注意，一般而言，智能指针放在头文件时，包含下目标对象的.h文件比较合适，不然不知道该分配多少空间。如果非要使用前置声明，其他cpp使用该.h文件时，必须包含智能指针的目标类型文件。或者还有一个解决方法，就是将带有智能指针的类显式的写上析构函数，这样其他函数在使用这个类时会链接到其定义，进而观察到智能指针的目标类型。==

- C++位字段写法，能显式定义字段占多少位，但这种字段成员无法取地址！（GCC4.9支持这种写法）
...
- 【前置声明】
  ...
  C++语义上区分：引入一个新类型还是引用现有类型，因此不支持在前置声明时使用::表示作用域，在前置声明一个带作用域的对象时必须同样写namespace包裹。

- .cc与.cpp等价，是C++源文件的另一种扩展名。

- 【" PRIu64"】" PRIu64" 是一个跨平台宏，表示无符号64位，print unsigned 64。

- 【C++ emplace_back】emplace_back本质是将传入的参数直接构造一个vector末尾构造一个元素类型的对象。==他有一个变参函数模版，因此如果传入两个迭代器，有可能是vector元素类型为列表，两个迭代器可以构造一个列表，进而直接放在末尾。==

- 智能指针：std::weak_ptr的意义是 我不拥有对象，但我想观察对象是否还存在，只要wak_ptr没死，他就必须能“观察”对象哪怕对象死掉（以及被释放了）。这个观察的行为是由control block对象负责。 要更新shared_ptr指代的对象，可以使用reset，本质是用一个临时的share_ptr(或者传入一个new对象)进行swap，然后把原油对象析构掉。
...
- `std::ofstream` 是 C++ 标准库中用于文件输出的类，可以通过构造函数直接打开文件std::ofstream outputFile("example.txt"); // 直接创建并打开文件。你可以使用 `<<` 操作符将数据写入文件。

  ```
  cppoutputFile << "Hello, World!" << std::endl; // 写入字符串
  outputFile << "The value of 42 is: " << 42 << std::endl; // 写入整数
  ```
...
- 【%0.0f%%】在格式化标记中，第一个是格式规范符的开始标志，表示后面有一个格式化插入点。0.0中第一个`0`告诉打印函数在实际数字前要填充零。然而，因为后面是 `.0` 的，函数不会填充任何零，因此这个 `0` 在此场合下是冗余的。`.0`表示要输出的小数位数为零，也就是说只会显示整数部分，且不显示小数部分。`%%`两个百分号是用来输出一个字面上的 `%` 符号。在格式化字符串中，`%%` 被解析为单一的 `%` 字符。

- std::ofstream C++提供的ofstream对象可以构造或者.open，其open有几种截断重写、追加等模式，默认是使用trunc，即截断重写模式。
...
- std::all_of, std::none_of和std::any_of【返回true或者false，判断区间内元素与判断条件的满足状况。】
...
- std::max(a, b)利用模板类型推导a,b中的较大值
- std::max_element(container)，返回容器中最大元素的迭代器
-  std::reverse用于反转在[first,last)范围内的顺序（包括first指向的元素，不包括last指向的元素），reverse函数**没有返回值。**
...
- 【添加交互】MFC添加交互资源可以直接用VS的资源视图，点进去.rc文件会有资源编辑器，里面会同步修改.rc和.h资源文件。点开对应的Dialog后，在左边的工具箱可以拖入对应的控件，右键设置其名称等属性。
...
- 【std::thread detach()与join()】detach和join都可以被用于启动被调线程，区别在于两者是否阻塞主调线程.

- 【生成唯一线程ID】

  ```
  static std::atomic<int32_t> _threadId(LANE_RENDER_THREAD_ID + 1);
  return _threadId.fetch_add(1);
  ```
...
- CPU核：每个核心一次可以执行一个线程，现在高性能核心可以有超线程技术，但移动soc没有。
- 线程：线程是操作系统调度的执行单位，每个线程不必单独占据核心。CPU会在多个线程之间快速切换执行权，这就是并发和并行的区别。
- 本质上CPU核是设备的计算能力单位，线程是一个子任务，线程数多于CPU核的意义在于，有些线程可能会被网络I/O, 磁盘读写、信号量等阻塞。此时本地的计算能力可以切换任务（线程）继续执行。
- 线程多核少的调度特点：
  - I/O密集型任务(比如网络请求、UI事件)可以用很多线程，因为大部分是被阻塞的状态，
  - CPU密集型任务（比如视频编码、复杂数学计算）即使开很多线程、核也不够，反而可能出现上下文切换开销而变慢。
  - Scheduler会根据优化线(实时、后台、interactive)核Qos(Quality of Service)抢占执行。

## 线程与内存

- 线程间传递资源时，注意不要传递某个栈上拿到的裸指针，因为无法控制资源的释放顺序。
- Std::shared_ptr本身的引用计数增加/减少时原子操作，线程安全。但是对象本身并不线程安全，不能同时多线程读写统一对象的内容。
- 想避免复制资源，但是不想传递shared_ptr后导致生命周期难以追踪
  - 1、用shared_ptr + weak_ptr，主线程管理资源，其他线程只持有weak_ptr，需要用时lock()
  - 主线程拥有唯一所有权，线程间传资源ID，不共享对象本身，其他线程通过ID从一个线程安全的资源池获取
  - 如果对象只读，用shared_ptr。
...
- 【模板参数，编译器计数有多少步骤】模板参数可以为类型参数与非类型参数，非类型参数除int外，还可以为指针，此时就可以直接将模板参数视为变量进行修改，同时提供了更好的类型信息。

  ~~~cpp
  // 提供编译期计数的能力
  template<typename T, Gint64 *Counter, Gint64 N>
  class StepCounter
  {
      static Gint64 AddStepCount()
      {
          return *Counter += N;
      }
  public:
      static Gint64 DoStepAdd;
  };
  
  template<typename T, Gint64 *Counter, Gint64 N>
  Gint64 StepCounter<T, Counter, N>::DoStepAdd = AddStepCount();
  #define StepCounterAdd(c,n)  do{                    \
      auto x = []() {};                               \
      StepCounter<decltype(x), &c, n>{}.DoStepAdd;    \
  }while (0)
  
  // 每一次调StepCounterAdd宏可以理解为创建一个临时对象尝试初始化DoStepAdd，而每一次**构造初始时会带有一个Add的行为，反过来修改模板参数**
  // 这样通过不断的实例化，模板指针参数会被不断的修改，编译完成后，该参数即完成了程序想要的计数【该参数为静态参数
  // 为什么会不断的实例化呢，是因为当我们使用 lambda 作为模板参数时，每个新定义的 lambda 会创建一个新的匿名类型】。
  ~~~
~~~
...
- \_\_LINE\_\_：在源代码中插入当前源代码行号，\_\_LINE\_\_仅反映当前源文件的实际行号，而不包括从头文件中引入的行号；
...
- `__COUNTER__` 是 C 和 C++ 中的一个预定义宏。它用于在编译过程中生成一个递增的整数值，通常用于创建唯一的标识符或有序的编号。一般可以用于生成匿名变量，比如

  ~~~cpp
  ANONYMOUS_VARIABLE(str)  CONCATENATE(str, __COUNTER__)这是生成一个匿名变量名名称的模板。
  ~~~

- 【宏处理方法调用】使用匿名函数的构造函数实现自定义的函数逻辑
  ...
  namespace detail
  {
      ...
      struct ExecOnConstruct { ExecOnCostruct(Func f) {f(); }};
      struct BraceAssist PExecOnConstruct operator+(Func&& f) {retrun ExecOnConstruct(f);}};
  }
  
  #define CONSTRUCT_EXEC \
  private:deyail::ExecOnConstruct ANONYMOUS_VARIABLE(EXEC_ON_CONSTRUCT) = detail::BraceAssist() + [&]()
  
  // 使用
  CONSTRUCT_EXEC{SetTableName(nam);};
  ~~~

- 【宏中使用do, while逻辑】宏中使用do while(0)是为了在宏内部提供一个块作用域，无论在代码中宏被什么上下文调用，都有一个一致的语法结构。

- C++利用宏写监视，可以使用\_\_line\_\_避免变量名重复，同时函数内字符串拼接等行为最好再用一个函数包裹比较合适。

  ~~~cpp
  #define GLD_SAFE_DELETE(ptr) \
  { \
  delete ptr; \
  ptr = nullptr; \
  } \
  ...
  #define GLD_SAFE_DELETE_GUARD(PTR) \
      auto CONCATENATE(Var_, __LINE__) = [&]() { GLD_SAFE_DELETE(PTR); }; \
      detail::ScopeGuard<decltype(CONCATENATE(Var_, __LINE__))> CONCATENATE(guard_, __LINE__)(std::move(CONCATENATE(Var_, __LINE__))); \
  ~~~

- 宏函数定义中，``\` 是续行符`，不会改变逻辑结构，因此可以用宏函数来封装宏定义。

- 要在宏函数中使用条件语句，最好就只使用常规的三元运算符，宏函数里面不能生成预处理指令，因为层级运行的，宏函数展开前，预处理指令会已经处理完了，除了微软的#pragma，他提供了_Pragma操作符模拟。

  ~~~cpp
  #define UPPER_FADING_DEBUG            0
  #define DIVERSION_ESTIMATION_DEBUG    0
  #define UD_JUNCTION_DEBUG             1
  
  #define ROAD_GET_OPTIMIZE_STATE(NAME) \
      (NAME##_DEBUG ? "off" : "on")
  
  #define ROAD_STRINGIFY(x) #x
  
  // 生成优化指令的主宏，根据某个宏变量是否被打开来生成优化选项。
  #define OPTIMIZE_SETTING_MACRO(NAME) \
      _Pragma(ROAD_STRINGIFY(optimize("", ROAD_GET_OPTIMIZE_STATE(NAME))))
  ~~~

- 【调试宏】要调试宏是否按预期展开，在VS中可以右键项目—属性—C/C++—预处理器—输出到文件，此时便后某个文件时不会生成.obj，而是会在相同目录下生成.i文件，在文件中搜索内容查看是否正确即可。

- 在宏展开时，##拼接优先级高于宏替换，如果希望宏替换优于##，需要定义中间宏。最差情况下，一个宏函数块一次只会进行一次展开，如果一个宏函数期望带有多个展开的情况下，最好也像第一层那样，做几个强制中间宏。

  ~~~cpp
  // 第一层：间接处理参数展开
  #define ROAD_GET_OPTIMIZE_STATE_IMPL(VALUE) \
      ROAD_GET_OPTIMIZE_STATE_INDIRECT(VALUE)  // 先展开 VALUE
  
  // 第二层：实际拼接宏名称
  #define ROAD_GET_OPTIMIZE_STATE_INDIRECT(VALUE) \
      ROAD_GET_OPTIMIZE_STATE_##VALUE
  
  // 主入口宏
  #define ROAD_GET_OPTIMIZE_STATE(NAME) \
      ROAD_GET_OPTIMIZE_STATE_IMPL(NAME##_DEBUG)
